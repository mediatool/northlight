import React from 'react'
import ReferencePage from '../../reference-page'

const PPage = () => (
  <ReferencePage data={ {
    tags: {},
    filePath: '/Users/sebastian.delgado/Documents/Programmering/Mediatool/northlight/framework/lib/components/typography/body/p.tsx',
    description: '',
    displayName: 'P',
    methods: [],
    props: {
      variant: {
        defaultValue: {
          value: '16',
        },
        description: '',
        name: 'variant',
        parent: {
          fileName: 'framework/lib/components/typography/body/types.ts',
          name: 'PProps',
        },
        declarations: [
          {
            fileName: 'framework/lib/components/typography/body/types.ts',
            name: 'PProps',
          },
        ],
        required: false,
        type: {
          name: 'enum',
          raw: '"14" | "16"',
          value: [
            {
              value: '"14"',
            },
            {
              value: '"16"',
            },
          ],
        },
      },
      align: {
        defaultValue: null,
        description: 'The CSS `text-align` property',
        name: 'align',
        parent: {
          fileName: 'northlight/node_modules/@chakra-ui/layout/dist/text.d.ts',
          name: 'TextProps',
        },
        declarations: [
          {
            fileName: 'northlight/node_modules/@chakra-ui/layout/dist/text.d.ts',
            name: 'TextProps',
          },
        ],
        required: false,
        type: {
          name: 'SystemProps["textAlign"]',
        },
      },
      decoration: {
        defaultValue: null,
        description: 'The CSS `text-decoration` property',
        name: 'decoration',
        parent: {
          fileName: 'northlight/node_modules/@chakra-ui/layout/dist/text.d.ts',
          name: 'TextProps',
        },
        declarations: [
          {
            fileName: 'northlight/node_modules/@chakra-ui/layout/dist/text.d.ts',
            name: 'TextProps',
          },
        ],
        required: false,
        type: {
          name: 'SystemProps["textDecoration"]',
        },
      },
      casing: {
        defaultValue: null,
        description: 'The CSS `text-transform` property',
        name: 'casing',
        parent: {
          fileName: 'northlight/node_modules/@chakra-ui/layout/dist/text.d.ts',
          name: 'TextProps',
        },
        declarations: [
          {
            fileName: 'northlight/node_modules/@chakra-ui/layout/dist/text.d.ts',
            name: 'TextProps',
          },
        ],
        required: false,
        type: {
          name: 'SystemProps["textTransform"]',
        },
      },
    },
  } }
  />
)

export default PPage
